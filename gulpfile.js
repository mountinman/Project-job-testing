...
gulp.task("css", function() {
  gulp
    .src("src/css/*.css")
    .pipe(
      postcss([require("postcss-normalize")({ browsers: "last 3 versions" })])
    )
    .pipe(gulp.dest("dist/css"));
});
...
gulp.task("serve", ["sass"], function() {
  ...
  gulp.watch(["src/scss/features/*.scss"], ["sass"]);
  gulp.watch(["src/scss/breakpoints/*.scss"], ["sass"]);
  gulp.watch(["src/scss/layout/*.scss"], ["sass"]);
  gulp.watch(["src/scss/elements/*.scss"], ["sass"]);
  gulp.watch("src/*.html").on("change", browserSync.reload);
});

// Default Task
gulp.task("default", ["serve"]);
gulp.task("sendDist", ["copyHtml", "script", "dist", "css", "imageMin"]);
